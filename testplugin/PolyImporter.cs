...
namespace FrooxEngine.LogiX
{
    ...
    public class ModelImporterWithArgument : LogixNode
    {
        ...
        public void Run()
        {
            ...
            this.World.RootSlot.ChildRemoved += CheckIfImportIndicatorsRemoved;
        }

        void CheckIfImportIndicatorsRemoved(Slot slot, Slot child)
        {
            List<Slot> indicators = this.World.AllSlots.ToList().Where((Slot s) => s.Name == "Import indicator").ToList();
        ...
        private async Task WaitForImport()
        {
            ...
            {
                //List<Slot> indicators = this.World.AllSlots.ToList().Where((Slot s) => s.Name == "Import indicator").ToList();
                //UniLog.Log(indicators);
                //while (indicators.Count > 0) { 
                //    indicators = this.World.AllSlots.ToList().Where((Slot s) => s.Name == "Import indicator").ToList();
                //    //indicator = indicators.Last();
                //    //UniLog.Log(indicator);
                //}
                UniLog.Log("HIIIIIIIIIIIIIIIIIIII");
                Slot new_object = World.RootSlot.Children.ToList()[World.RootSlot.ChildrenCount - 1];
                MessageManager.UserMessages userMessages = this.Engine.Cloud.Messages.GetUserMessages("U-guillefix2");
                ObjectMessageData messageData = await EngineCloudXExtensions.CreateObjectMessage(userMessages, new_object, true);
                await userMessages.SendMessage(messageData.message);
                await messageData.uploadTask;
                UniLog.Log(messageData.message.Content);
                Record record = messageData.message.ExtractContent<Record>();
                UniLog.Log(record.AssetURI);
                OnDone.Trigger();
            });
        }
    }
